...
class FriendsTableViewCell: UITableViewCell {
    ...
    func configure(user: UserModel) {
        friendNameLabel.text = user.getFullName()
        fritendAvatarImage.sd_setImage(with: URL(string: user.avatar), placeholderImage: UIImage(named: "glasses"))
        savedObject = user

    }
    
    //MARK: - Метод анимации нажатия на ячейку
