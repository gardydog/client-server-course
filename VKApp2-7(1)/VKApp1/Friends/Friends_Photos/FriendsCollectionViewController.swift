...
class FriendsCollectionViewController: UICollectionViewController {
    ...
    func setPhotosBy(userId: Int) {
        networkservice.loadFriendsPhotos(by: userId) { [weak self] in
            guard let self = self else { return }
            
            if let photos = self.realmManager.read(object: PhotoModel.self, filter: "ownerID == \(userId)") as? [PhotoModel] {
                self.photos = photos
